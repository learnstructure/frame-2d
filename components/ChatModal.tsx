...
const HOURLY_LIMIT = 20; // Increased limit for improved UX
...
const ChatModal: React.FC<ChatModalProps> = ({ isOpen, onClose, model, initialResults }) => {
  ...
  const checkRateLimits = (): boolean => {
    ...
    try {
      ...
      if (usage.count >= HOURLY_LIMIT) {
        setRateLimitError(`You have reached the hourly limit of ${HOURLY_LIMIT} messages.`);
        return false;
      }
  ...
  const handleSend = async () => {
    ...
    if (messages.length > 0 && messages[messages.length - 1].role === 'user' && messages[messages.length - 1].content === input) {
      setRateLimitError("Duplicate message detected.");
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1e293b] w-full max-w-2xl h-[85vh] md:h-[600px] rounded-xl shadow-2xl border border-slate-700 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div>
              <h3 className="font-bold text-white flex items-center gap-2">
                ...
                <select
                  ...
                  className="ml-2 text-[10px] md:text-xs bg-slate-800 border border-slate-600 rounded px-2 py-1 text-slate-300 outline-none focus:border-cyan-500"
                >
              ...
              <p className="text-[10px] md:text-xs text-slate-400">
                {provider === 'gemini' ? 'Gemini 3 Flash' : 'Llama 3.3 70B'}
              </p>
            </div>
          </div>
        ...
        {/* Chat Area */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#1e293b] min-h-0">
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={`max-w-[85%] md:max-w-[80%] rounded-lg p-3 text-sm leading-relaxed ${msg.role === 'user'
        ...
        <div className="p-4 border-t border-slate-700 bg-[#0f172a] flex-shrink-0">
          <div className="flex gap-2 relative">
            <input
              ...
              placeholder={cooldown > 0 ? `Wait ${Math.ceil(cooldown / 1000)}s...` : "Ask a question..."}
              className="flex-1 bg-slate-800 border border-slate-600 rounded-lg px-4 py-2.5 text-sm text-white focus:border-cyan-500 outline-none disabled:opacity-50 disabled:cursor-not-allowed shadow-inner"
            />
            <button
              onClick={handleSend}
              disabled={loading || !input.trim() || cooldown > 0}
              className={`px-4 py-2.5 rounded-lg transition-colors text-white disabled:opacity-50 disabled:cursor-not-allowed min-w-[50px] flex items-center justify-center shadow-lg active:scale-95 ${provider === 'gemini' ? 'bg-cyan-600 hover:bg-cyan-500' : 'bg-orange-600 hover:bg-orange-500'}`}
            >
              {cooldown > 0 ? (
          ...
          {rateLimitError && (
            <p className="mt-2 text-center text-red-400 text-[10px] animate-pulse">
              {rateLimitError}
            </p>
          )}
        </div>
